...
export default function RetailForm() {
  ...
  const updateMedicineRow = (index, field, value) => {
    ...
    // SMART TABLET POTENCY AUTO-LOGIC:
    // If bottle size is changed to tablets, automatically select 'N/A' (None) for Potency since tablets don't require dilutions.
    if (field === 'bottleSize') {
      if (value.includes('Tablets')) {
        updated[index].potency = 'N/A';
      } else if ((value === '30ml' || value === '100ml' || value === '450ml') && updated[index].potency === 'N/A') {
        // If switched back to liquid, reset from N/A back to standard 30C dilution
        updated[index].potency = '30C';
      }
    }

    setMedicineItems(updated);
    // Reset pricing estimates if items changed to keep UI in sync
  ...
  const validate = () => {
    ...
    medicineItems.forEach((itm, idx) => {
      if (!itm.name.trim()) {
        itemErrors[idx] = 'Medicine name is required.';
      }
      if (itm.potency === 'custom' && !itm.customPotency.trim()) {
        itemErrors[idx] = 'Please specify custom potency.';
      }
      if (itm.bottleSize === 'custom' && !itm.customBottleSize.trim()) {
        itemErrors[idx] = 'Please specify custom bottle size.';
      }
    });
  ...
  const handleEstimatePrices = async () => {
    // Basic verification before AI estimation
    const firstEmptyIndex = medicineItems.findIndex(itm => !itm.name.trim());
    if (firstEmptyIndex !== -1) {
      setEstimateError(`Please fill out the name of all medicines first (check row ${firstEmptyIndex + 1}).`);
      return;
    }
  ...
  const handleRetailSubmit = async (e) => {
    ...
    // If customer hasn't requested AI price estimate, run it silently or use a fallback estimation
    ...
    if (!currentEstimate) {
      ...
      // Hard defaults if estimate service completely failed
      if (!currentEstimate) {
        ...
        const fallbackItems = medicineItems.map(itm => {
          ...
          return { name: `${itm.name}${formattedPotency} (${size})`, quantity: itm.quantity, estimatedUnitPrice: 120, totalPrice: total };
        });
    ...
    // Format list into clean summary for Google Sheets and WhatsApp
    const medicinesSummary = medicineItems
      .map(itm => {
        const potency = itm.potency === 'custom' ? itm.customPotency : itm.potency;
        const formattedPotency = potency === 'N/A' ? '' : ` ${potency}`;
        const size = itm.bottleSize === 'custom' ? itm.customBottleSize : itm.bottleSize;
        return `• ${itm.name}${formattedPotency} (${size}) [Qty: ${itm.quantity}]`;
      })
      .join('\n');
    ...
    try {
      ...
      setError('An unexpected error occurred. Please verify your details and try again.');
    } finally {
      setSubmitting(false);
    }
  };
        ...
        <div className="lg:col-span-4 flex flex-col justify-between">
          <div className="space-y-4">
            ...
            <p className="text-sm text-[#64748B] leading-relaxed">
              Build your customized medicine request by selecting the exact homeopathic **dilution power (potency)** and **bottle size**. Our real-time AI will ground itself with local Ranchi pricing to prepare your estimate.
            </p>
          </div>
          ...
          <div className="mt-8 space-y-4 border-t border-[#EAE5DC] pt-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h4 className="text-xs font-bold text-[#1E293B] uppercase tracking-wider">All Ranchi Delivery</h4>
                <p className="text-2xs text-[#64748B]">Serving all pin codes inside Ranchi city limits.</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h4 className="text-xs font-bold text-[#1E293B] uppercase tracking-wider">24 Hour Delivery</h4>
                <p className="text-2xs text-[#64748B]">Orders are shipped and delivered within 24 hours.</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h4 className="text-xs font-bold text-[#1E293B] uppercase tracking-wider">Free Shipping above ₹500</h4>
                <p className="text-2xs text-[#64748B]">Otherwise, a flat delivery fee of just ₹50 is applied.</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h4 className="text-xs font-bold text-[#1E293B] uppercase tracking-wider">Upto 10% Off Remedies</h4>
                <p className="text-2xs text-[#64748B]">Get exclusive discounts on retail medicine purchases.</p>
              </div>
            </div>
          </div>
        </div>
          ...
          {success && orderDetails ? (
            <div className="animate-fade-in space-y-6">
              <div className="p-4 bg-emerald-50 border border-emerald-200 rounded-xl text-emerald-800 flex items-start gap-3">
                ...
                <div>
                  <h4 className="text-sm font-bold">Retail Order Logged!</h4>
                  <p className="text-xs text-[#64748B] mt-0.5">Your medicine order is registered and synced with our pharmacy spreadsheet successfully.</p>
                </div>
              </div>
              ...
              <div className="bg-[#FDFBF7] border border-[#EAE5DC] rounded-2xl p-5 md:p-6 space-y-4 shadow-sm relative overflow-hidden">
                ...
                <h4 className="font-extrabold text-sm text-[#115E59] uppercase tracking-wider border-b border-[#EAE5DC] pb-2 flex items-center gap-2">
                  <span>📋</span> Retail Order Slip Summary
                </h4>
                
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                  <div>
                    <span className="block text-[10px] font-bold text-slate-400 uppercase">Customer Name</span>
                  ...
                  <div className="md:col-span-2">
                    <span className="block text-[10px] font-bold text-slate-400 uppercase">Delivery Address</span>
                    <span className="font-semibold text-slate-800">{orderDetails.address}</span>
                  </div>
                  <div className="md:col-span-2">
                    <span className="block text-[10px] font-bold text-slate-400 uppercase">Medicines Requested</span>
                ...
                <div className="border-t border-[#EAE5DC] pt-3 mt-4 space-y-2 text-xs">
                  <div className="flex justify-between text-slate-600">
                    <span>AI Estimated Subtotal:</span>
              ...
              <div className="p-4 bg-teal-50/50 border border-teal-100 rounded-xl flex items-start gap-2.5 text-xs text-[#0F766E] leading-relaxed">
                <span>💡</span>
                <div>
                  <strong className="text-teal-900 font-bold block mb-0.5">Instant Dispatch verification</strong>
                  Your order is booked securely. Send details straight to our pharmacist via WhatsApp for immediate courier matching and dispatch!
                </div>
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                <a
                  ...
                  className="flex-1 py-3 px-6 bg-[#25D366] hover:bg-[#1EBE57] text-white rounded-xl font-bold text-xs uppercase tracking-wider text-center shadow-md transition-all flex items-center justify-center gap-2"
                >
            ...
            <form onSubmit={handleRetailSubmit} className="space-y-5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div className="md:col-span-2">
                  <label htmlFor="retail-address" className="block text-2xs font-bold text-[#64748B] uppercase tracking-widest mb-1.5">
                    Full Delivery Address in Ranchi
                  </label>
                  <div className="relative">
                    ...
                    <textarea
                      ...
                      placeholder="e.g. Flat 302, Green Glen Apartments, Lalpur, Ranchi, Jharkhand 834001"
                      ...
                      >
                        ...
                        {medicineItems.length > 1 && (
                          <button
                            ...
                            className="absolute top-3 right-3 text-slate-400 hover:text-rose-600 transition-colors p-1 rounded-lg hover:bg-rose-50"
                        ...
                        <div className="text-[10px] font-extrabold text-[#115E59] uppercase tracking-wider flex items-center gap-1.5">
                          <span>📦</span> Medicine Item #{index + 1}
                        </div>

                        {/* Input Row Grid with responsive, spacious layout to prevent cut-off details */}
                        <div className="grid grid-cols-1 md:grid-cols-12 gap-3.5">
                          ...
                          <div className="md:col-span-12 lg:col-span-4">
                            ...
                            <input
                              ...
                              placeholder="e.g. Arnica, Nux Vomica, Ashwagandha"
                          ...
                          <div className="md:col-span-5 lg:col-span-3">
                            <label className="block text-[9px] font-bold text-slate-400 uppercase mb-1">Potency (Power)</label>
                            <select
                              value={item.potency}
                            ...
                            >
                              ...
                              <option value="N/A">None / N/A (for Tablets/Trits)</option>
                          ...
                          {/* Bottle Size */}
                          <div className="md:col-span-5 lg:col-span-3">
                            <label className="block text-[9px] font-bold text-slate-400 uppercase mb-1">Bottle Size</label>
                            <select
                              value={item.bottleSize}
                              onChange={(e) => updateMedicineRow(index, 'bottleSize', e.target.value)}
                              className="block w-full px-3 py-2 text-xs bg-white border border-[#EAE5DC] rounded-xl text-[#1E293B] focus:outline-none focus:ring-2 focus:ring-[#115E59]/40 transition-all shadow-sm"
                            >
                              <option value="30ml">30 ml (Standard Liquid)</option>
                              <option value="100ml">100 ml (Large Liquid)</option>
                              <option value="450ml">450 ml (Clinic/Bulk Liquid)</option>
                          ...
                          <div className="md:col-span-2 lg:col-span-2">
                            ...
                            <div className="flex items-center bg-white border border-[#EAE5DC] rounded-xl overflow-hidden shadow-sm h-8.5 w-full">
                              <button
                                ...
                                className="flex-1 text-center font-bold text-slate-500 hover:bg-slate-50 active:bg-slate-100 transition-colors h-full flex items-center justify-center border-r border-[#EAE5DC]"
                              >
                                -
                              </button>
                              ...
                              <button
                                ...
                                className="flex-1 text-center font-bold text-slate-500 hover:bg-slate-50 active:bg-slate-100 transition-colors h-full flex items-center justify-center border-l border-[#EAE5DC]"
                              >
                                +
                              </button>
                            </div>
              ...
              <div className="bg-[#F9F6F0]/65 border border-[#EAE5DC] rounded-xl p-4.5 space-y-3 relative overflow-hidden transition-all shadow-sm">
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <h4 className="text-xs font-bold text-[#1E293B] uppercase tracking-wider">AI Price Estimator</h4>
                      <p className="text-[10px] text-slate-500">Estimate pricing instantly using AI before buying.</p>
                    </div>
                  </div>
                  <button
                    type="button"
                    ...
                    className="py-1.5 px-4 bg-teal-50 border border-teal-200 text-[#0F766E] rounded-lg text-2xs font-bold uppercase tracking-wider hover:bg-teal-100/60 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center justify-center gap-1.5"
                  >
                    {estimating ? (
                      <>
                ...
                {estimatedData && (
                  <div className="bg-white border border-[#EAE5DC] rounded-lg p-3.5 text-2xs space-y-3 animate-fade-in shadow-inner">
                    <h5 className="font-extrabold text-[10px] text-[#115E59] uppercase tracking-wider border-b border-slate-100 pb-1">AI Price Estimates Breakdown:</h5>
                    ...
                    <div className="text-[9px] text-slate-400 leading-relaxed italic border-t border-slate-100 pt-1.5">
                      💡 {estimatedData.explanation} Price estimates are approximate and based on standard Indian homeopathic pricing lists.
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="pt-2">
                <button
                  type="submit"
                  disabled={submitting}
                  className="w-full btn-neon-emerald py-3 px-6 rounded-xl flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed uppercase text-xs tracking-wider font-bold shadow-md hover:shadow-lg active:scale-95"
                >
                  {submitting ? (
                    <>
